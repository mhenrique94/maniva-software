...
import { company, quickLinks, connections, footerBaseline, lastUpdated } from "./footer.js";

test("company: marca + razón social + CNPJ correctos (§5.8/CONTEXT)", () => {
...
test("quickLinks: todos los enlaces excepto 'Inicio'", () => {
  const links = quickLinks();
  assert.ok(!links.some((l) => l.id === "inicio"));
  assert.deepEqual(
    links.map((l) => l.id),
    ["pmes", "empresas", "projetos", "metodo", "sobre"],
  );
});

test("connections: HelpMed + LinkedIn externos con ancla rica", () => {
  assert.deepEqual(
...
test("footerBaseline: © año + tagline rizomático", () => {
